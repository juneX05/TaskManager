<template>
  <app-layout>
    ...
    <v-col cols="12">
      <v-row>
        <v-col cols="12">
          <inertia-link :href="route('createProjectStatus')" as="v-btn" class="float-end" small>
            <v-icon>add</v-icon>
            Add ProjectStatus
          </inertia-link>
        </v-col>
        ...
        >
          ...
          <template v-slot:item.actions="{ item }">
            <div v-if="item_id == null ">
              <inertia-link :href="route('viewProjectStatus',[item.id])" as="v-icon" class="mr-2" small>
                mdi-eye
              </inertia-link>
              <inertia-link :href="route('editProjectStatus',[item.id])" as="v-icon" class="mr-2" small>
                mdi-pencil
              </inertia-link>
              <v-icon
                  small
...
export default {
  ...
  methods: {
    remove() {
      this.$inertia.post(route('deleteProjectStatus'), {
        id: this.item_id
      }, {
