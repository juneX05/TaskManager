<template>
  <app-layout>
    ...
    <v-col cols="12">
      <v-row>
        <v-col cols="12">
          <inertia-link :href="route('createTask')" as="v-btn" class="float-end" small>
            <v-icon>add</v-icon>
            Add Task
          </inertia-link>
        </v-col>
        ...
        >
          ...
          <template v-slot:item.actions="{ item }">
            <div v-if="item_id == null ">
              <inertia-link :href="route('viewTask',[item.id])" as="v-icon" class="mr-2" small>
                mdi-eye
              </inertia-link>
              <inertia-link :href="route('editTask',[item.id])" as="v-icon" class="mr-2" small>
                mdi-pencil
              </inertia-link>
              <v-icon
                  small
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
				 ...
				 { text: 'Start Date', align: 'start', sortable: true, value: 'start_date', }, 
				 { text: 'Completion Date', align: 'start', sortable: true, value: 'completion_date', }, 
				 { text: 'Task Status', align: 'start', sortable: true,  value: 'task_status.name', },  
				 { text: 'Project', align: 'start', sortable: true,  value: 'project.name', },  
  ...
  methods: {
    remove() {
      this.$inertia.post(route('deleteTask'), {
        id: this.item_id
      }, {
