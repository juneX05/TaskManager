<template>
  <initial-layout>
    <template #header>
      <inertia-link as="button" class="btn btn-primary btn-sm float-right" :href="route('viewProjectModuleAction', [action.id])">
    ...
    methods: {
        submit() {
            ...
            this.$inertia.form(this.form)
                ...
                .post(this.route('saveProjectModuleActionTask'), {
                    onSuccess: () => {
                        this.$inertia.visit(route('viewProjectModuleAction', [ this.action.id]))
                    },
                    onError: () => {
