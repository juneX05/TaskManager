  ...
  <div>
    ...
    <div class="card">
      <div class="card-header">
        <h3 class="card-title float-left">Actions</h3>

        <inertia-link as="button" class="btn btn-primary btn-sm float-right" :href="route('createProjectModuleAction',[module_id])">
        ...
        <table class="table table-striped actions">
          ...
          <tr v-for="(item, index) in data" :key="index">
            ...
            <td class="action-actions text-right">
              ...
              <inertia-link class="btn btn-primary btn-sm" :href="route('viewProjectModuleAction',[item.id])">
                <i class="fas fa-folder">
                </i>
                View
              </inertia-link>

              <inertia-link class="btn btn-info btn-sm" :href="route('editProjectModuleAction',[item.id])">
...
export default {
  name:"ActionsIndex",
    ...
    methods: {
        remove() {
            this.$inertia.post(route('deleteProjectModuleAction'), {
                id: this.item_id
            },{
                onFinish: () => {
                  this.item_id = null;
